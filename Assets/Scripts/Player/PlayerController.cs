...
public class PlayerController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        // --- Microcontroller / Joystick serial input (U D R L) ---------------
        if (SerialInputManager.Instance != null)
        {
            Debug.Log("SerialInputManager Set");
            Vector3 serialDir = SerialInputManager.Instance.ConsumePendingDirection();
            if (serialDir != Vector3.zero)
                nextDirection = serialDir;
        }
